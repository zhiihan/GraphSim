...
Stabilizer::Stabilizer(const VertexIndex numQubits_)
    : paulis(numQubits_, vector<LocCliffOp>(numQubits_, lco_Id)),
      rowsigns(numQubits_, rp_p1), vtxidx(numQubits_, 0) {
...
Stabilizer::Stabilizer(struct QState *qs)
    : paulis(qs->n, vector<LocCliffOp>(qs->n, lco_Id)), rowsigns(qs->n),
      vtxidx(qs->n) {
    ...
    for (int i = 0; i < (int)numQubits; i++) {
        ...
        for (int j = 0; j < (int)numQubits; j++) {
            bool xhere =
                ((qs->x[numQubits + i][j >> 5]) & (1 << (j & 0x1f))) > 0;
            bool zhere =
                ((qs->z[numQubits + i][j >> 5]) & (1 << (j & 0x1f))) > 0;
...
void Stabilizer::conjugate(unsigned row, unsigned col, const LocCliffOp trans) {
...
bool Stabilizer::compare(const Stabilizer &othr) {
    ...
    for (int r = 0; r < numQubits; r++) {
        for (int c = 0; c < numQubits; c++) {
            ...
            othrmatr[numQubits * 2 * r + c] =
                (othr.paulis[r][c] == lco_X || othr.paulis[r][c] == lco_Y) ? 1
                                                                           : 0;
            othrmatr[numQubits * (2 * r + 1) + c] =
                (othr.paulis[r][c] == lco_Z || othr.paulis[r][c] == lco_Y) ? 1
                                                                           : 0;
        }
    }
    ...
    for (int r = 0; r < numQubits; r++) {
        for (int c = 0; c < numQubits; c++) {
            assert(trans[r * numQubits + c] == 0 ||
                   trans[r * numQubits + c] == 1);
            if (trans[r * numQubits + c]) {
                ...
                for (int cc = 0; cc < numQubits; cc++) {
                    ...
                    trstab.rowsigns[r] =
                        trstab.rowsigns[r] +
                        LocCliffOp::mult_phase(trstab.paulis[r][cc],
                                               paulis[c][cc]);
                }
            }
        }
    ...
    for (int r = 0; r < numQubits; r++) {
        if (othr.rowsigns[r] != trstab.rowsigns[r]) {
            DBGOUT("Stabilizer differ: Mismatch of rowsigns in row " << r
                                                                     << endl);
            return false;
        }
        for (int c = 0; c < numQubits; c++) {
