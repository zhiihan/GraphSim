...
struct RightMatrix {
    ...
    bool apply_on_state(vector<bool>::reference ampl1,
                        vector<bool>::reference ampl2, RightPhase &ph1,
                        RightPhase &ph2);
};
#endif
...
inline LocCliffOp::LocCliffOp(unsigned short int op_) { op = op_; }
...
inline bool LocCliffOp::is_diagonal(void) const {
    return (op == lco_Id.op || op == lco_Z.op || op == lco_smiZ.op ||
            op == lco_spiZ.op);
}
...
inline bool operator==(LocCliffOp a, LocCliffOp b) { return a.op == b.op; }

inline bool operator!=(LocCliffOp a, LocCliffOp b) { return a.op != b.op; }

inline RightPhase::RightPhase(void) { ph = 0; }

inline RightPhase::RightPhase(unsigned short ph_) { ph = ph_; }
