...
//! A quantum register.
/*! GraphRegister is the central class of graphsim. It represents a register of
qubits that can be entangled with each other. It offers functions to initialize
...
inline void GraphRegister::hadamard(VertexIndex v) { local_op(v, lco_H); }

/*! Apply a phaserot gate on vertex v. Phaserot means the gate
S = |0><0| + i |1><1|. */
inline void GraphRegister::phaserot(VertexIndex v) { local_op(v, lco_S); }

/*! Apply a bitflip gate (i.e. a Pauli X) on vertex v */
inline void GraphRegister::bitflip(VertexIndex v) { local_op(v, lco_X); }
