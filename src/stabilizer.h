...
struct QState {
  long n;            // # of qubits
  unsigned long **x; // (2n+1)*n matrix for stabilizer/destabilizer x bits
                     // (there's one "scratch row" at
  unsigned long **z; // (2n+1)*n matrix for z bits the bottom)
  int *r; // Phase bits: 0 for +1, 1 for i, 2 for -1, 3 for -i.  Normally either
          // 0 or 2.
...
/* This class is used at the moment only by GraphRegister::print_stabilizer and
for the compare method (when comparing with CHP). */
struct Stabilizer {
