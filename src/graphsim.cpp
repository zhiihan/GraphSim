...
//! Instantiate a quantum register with 'numQubits' qubits, initally all in
//! state |0>.
/*! If randomize > -1 the RNG will be seeded with the current time plus the
value of randomize. (Otherwise, it is not seeded.) That the value of randomize
is added to the seed is useful in parallel processing settings where you want to
...
//! Prints the line for Vertex i in the adjacency list representation of the
//! state.
void GraphRegister::print_adj_list_line(ostream &os, VertexIndex i) const {
  os << "Vertex " << i << ": VOp " << vertices[i].byprod.get_name()
     << ", neighbors ";
...
/*! Use the cphase look-up table. This is called by cphase after VOps that do
not commute with the cphase gate have been removed as far as possible. */
...
/*! Check whether the qubits are connected to each other and to non-operand
 * vertices.*/
ConnectionInfo GraphRegister::getConnectionInfo(VertexIndex v1,
                                                VertexIndex v2) {
  ConnectionInfo ci;
  ci.wasEdge = vertices[v1].neighbors.find(v2) != vertices[v1].neighbors.end();
  if (!ci.wasEdge) {
...
//! This structure is needed only by toggle_edges
struct edge_hash {
  size_t operator()(const Edge &e) const { return e.first << 16 ^ e.second; };
};
...
void GraphRegister::toggle_edges(const unordered_set<VertexIndex> vs1,
  ...
  for (VtxIdxIterConst i = vs1.begin(); i != vs1.end(); i++) {
    for (VtxIdxIterConst j = vs2.begin(); j != vs2.end(); j++) {
      if ((*i != *j) && (procd_edges.find(Edge(*i, *j)) == procd_edges.end())) {
...
int GraphRegister::graph_X_measure(VertexIndex v, bool *determined, int force) {
...
/* For basis, pass a LocCliffOp object which has to be equal to one of lco_X,
lco_Y or lco_Z. If you want to now, whether the result was choosen at random or
determined by the state, pass a bool pointer in which this information will be
written. If you want to force the result to be a certain value, pass 0 or 1 to
'force'. This only works, if the result is not determined. If it is, 'force' is
ignored.*/
int GraphRegister::measure(VertexIndex v, LocCliffOp basis, bool *determined,
  ...
  switch (basis.op) {
  case 1 /* lco_X */:
    res = graph_X_measure(v, determined, force);
    break;
  case 2 /* lco_Y */:
...
/* This changes the state's graph representation but not the state itself, as
the necessary correction to the VOps are applied. */
...
bool GraphRegister::remove_byprod_op(VertexIndex v, VertexIndex avoid) {
  ...
  const char *comp_tbl[24] = {"UUUU", "UU",   "VVUU", "VV",    "VUU",   "V",
                              "VVV",  "UUV",  "UVU",  "UVUUU", "UVVVU", "UUUVU",
                              "UVV",  "VVU",  "UUU",  "U",     "VVVU",  "UUVU",
                              "VU",   "VUUU", "UUUV", "UVVV",  "UV",    "UVUU"};
